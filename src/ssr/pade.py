...
VALID_PADE_ORDERS = (2, 3, 4, 5, 6)


def validate_pade_order(n_pade: int) -> None:
    """Validate the supported rough Heston Padé approximation order."""
    if n_pade not in VALID_PADE_ORDERS:
...
def _h_pade_22(tau, a, params):
    ...
    kappa_tilde = kappa_over_eta - (0 + 1j) * rho * a
    aa = np.sqrt(a * (a + (0 + 1j)) + kappa_tilde**2)
    rm = kappa_tilde - aa

    # Calculate coefficients
    b1 = -a * (a + (0 + 1j)) / 2 * 1 / sp.gamma(1 + al)
    b2 = -b1 * kappa_tilde * eta * sp.gamma(1 + al) / sp.gamma(1 + 2 * al)

    g0 = rm / eta
    g1 = 0 if al == 1 else -1 / aa * 1 / sp.gamma(1 - al) * g0 / eta
...
def _h_pade_33(tau, a, params):
    ...
    b2 = -b1 * kappa_tilde * eta * sp.gamma(1 + al) / sp.gamma(1 + 2 * al)
    b3 = (
        (-b2 * kappa_tilde * eta + eta**2 * b1**2 / 2)
        * sp.gamma(1 + 2 * al)
        / sp.gamma(1 + 3 * al)
    )

    g0 = rm / eta
    g1 = -1 / (aa * eta) * 1 / sp.gamma(1 - al) * g0
    g2 = (
        -1
        / (aa * eta)
        * (sp.gamma(1 - al) / sp.gamma(1 - 2 * al) * g1 - 0.5 * eta**2 * g1 * g1)
    )
...
def _h_pade_44(tau, a, params):
    ...
    b4 = (
        (-b3 * kappa_tilde * eta + eta**2 * b1 * b2)
        * sp.gamma(1 + 3 * al)
        / sp.gamma(1 + 4 * al)
    )

    g0 = rm / eta
    g1 = 0 if al == 1 else -1 / (aa * eta) * 1 / sp.gamma(1 - al) * g0
    g2 = (
        0
        if al == 1
        else -1
    ...
    g3 = (
        ...
        / (aa * eta)
        * (sp.gamma(1 - 2 * al) / sp.gamma(1 - 3 * al) * g2 - eta**2 * g1 * g2)
    )
...
def _h_pade_55(tau, a, params):
    ...
    b5 = (
        (-b4 * kappa_tilde * eta + eta**2 * (1 / 2 * b2 * b2 + b1 * b3))
        * sp.gamma(1 + 4 * al)
        / sp.gamma(1 + 5 * al)
    )
    ...
    g2 = (
        ...
        / (aa * eta)
        * (sp.gamma(1 - al) / sp.gamma(1 - 2 * al) * g1 - 1 / 2 * eta**2 * g1 * g1)
    )
    g3 = (
        0
        if al == 1
...
def _h_pade_66(tau, a, params):
    ...
    b6 = (
        (-b5 * kappa_tilde * eta + eta**2 * (b2 * b3 + b1 * b4))
        * sp.gamma(1 + 5 * al)
        / sp.gamma(1 + 6 * al)
    )
...
def _h_pade(tau, a, params, n_pade: int):
    ...
    """
    validate_pade_order(n_pade)
    return _H_PADE_BY_ORDER[n_pade](tau, a, params)
...
def _deriv_h_pade(tau, a, params, n_pade: int):
    ...
    return 0.5 * (eta * h_pade_n - rm) * (eta * h_pade_n - rp)
...
def g(a, t, params, n_pade):
    ...
    """
    kappa = params["kappa"]
    return kappa * _h_pade(t, a, params, n_pade) + _deriv_h_pade(t, a, params, n_pade)
